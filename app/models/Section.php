		...
		foreach($dbData as $key=>$section)
		{
			$sections[$i]=array(
				'text'=>($section->type==1?'<img src="/WE/images/13s.png" title="Раздел"> ':'').($section->type==2?'<img src="/WE/images/39s.png" title="Новости"> ':'').
						...
						($section->type==5?'<img src="/WE/images/53s.png" title="Отзывы"> ':'')."<a href=\"/wepanel/sections/act/edit/item/{$section->id}\">".$section->name."</a> ".
						($section->type!=1?'<a href="/wepanel/'.($section->type==2?'news':($section->type==3?'galleries':($section->type==4?'portfolio':($section->type==5?'guestbook':'')))).'" title="Перейти к содержимому"><img src="/WE/images/54s.png" title="Перейти к содержимому"></a> ':'').
						"<a href=\"/wepanel/sections/act/create/item/".$section->id."\" title=\"Добавить подраздел\"><img src=\"/WE/images/31s.png\"></a> "."<a href=\"/wepanel/sections/act/edit/item/".$section->id."\" title=\"Редактировать раздел\"><img src=\"/WE/images/20s.png\"></a> ".
						CHtml::ajaxLink(
							"<img src=\"/WE/images/33s.png\"> ",
							array('/wepanel/sections/act/delete', 'item' => $section->id),
