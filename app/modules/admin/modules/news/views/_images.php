				...
				?>
					<div style="float:left;padding:5px;margin-bottom:10px;margin-right:20px;background:#d3d3d3;width:322px" id="imgs<?=$images[$i]->id?>">
						...
						echo CHtml::ajaxLink(
							...
							array('/'.$this->module->id.'/'.Yii::app()->controller->id.'/updateimage'),
							array(
								'type' => 'GET', 
								'update' => '#msgs',
								'data'=>array(
						...
						<?if(Yii::app()->controller->id=='galleries'){?>Порядок: <input type="text" value="<?=$images[$i]->posled;?>" id="posled<?=$images[$i]->id;?>" size="3"><?}?>
